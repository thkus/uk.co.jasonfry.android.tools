...
public class VerticalSwipeView extends ScrollView {
    private static int DEFAULT_SWIPE_THRESHOLD = 500;
    ...
    private void initSwipeView() {
        Log.i("uk.co.jasonfry.android.tools.ui.VerticalSwipeView", "Initialising VerticalSwipeView");
        mLinearLayout = new LinearLayout(mContext);
        mLinearLayout.setOrientation(LinearLayout.VERTICAL);
        super.addView(mLinearLayout, 0, new LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT));
    ...
    public void addView(View child, int index) {
        ...
        if (child.getLayoutParams() == null) {
            params = new LayoutParams(LayoutParams.FILL_PARENT, mPageHeight);
        } else {
            params = child.getLayoutParams();
            params.height = mPageHeight;
        }
     ...
     */
    public void smoothScrollToPage(int page, boolean forward) {
        scrollToPage(page, true, forward);
    }

    private void scrollToPage(int page, boolean smooth, boolean forward) {
        int oldPage = mCurrentPage;
        /*if (page > getPageCount() || page < 0) {
            return;
        }*/

        if(forward && oldPage < getPageCount()){
            oldPage++;
        } else if(forward && oldPage > 0) {
            oldPage--;
        }

        if (smooth) {
            smoothScrollTo(0, oldPage * mPageHeight);
        } else {
            scrollTo(0, oldPage * mPageHeight);
        }
        mCurrentPage = page;
     ...
     * central
     *
     * @param pageHeight The width you wish to assign for each page
     * @return An integer to add to the left margin of the first child and the right margin of the last child
     */
    public int setPageHeight(int pageHeight) {
        mPageHeight = pageHeight;
        return (SCREEN_HEIGHT - mPageHeight) / 2;
    }
    ...
    public int calculatePageSize(MarginLayoutParams childLayoutParams) {
        return setPageHeight(childLayoutParams.topMargin + childLayoutParams.height + childLayoutParams.leftMargin);
    }
    ...
    public void setPageControl(PageControl pageControl) {
        ...
        pageControl.setOnPageControlClickListener(new OnPageControlClickListener() {
            public void goForwards() {
                smoothScrollToPage(mCurrentPage + 1, false);
            }
    ...
    private class SwipeOnTouchListener implements OnTouchListener {
        ...
        private boolean actionMove(MotionEvent event) {
            ...
            if (newDistance < 0) //down
            {
                newDirection = (mMotionStartY + 4 <= newDistance) ? 1 : -1;  //the distance +4 is to allow for jitter
            } else //up
            {
                newDirection = (mMotionStartY - 4 <= newDistance) ? 1 : -1;  //the distance -4 is to allow for jitter
            }
        ...
        private boolean actionUp(MotionEvent event) {
            ...
            float numberOfPages = mLinearLayout.getMeasuredHeight() / mPageHeight;
            float fingerUpPage = fingerUpPosition / mPageHeight;
            ...
            if (mPreviousDirection == 1) {
                ...
                forward = true;
                if (mDistanceY < DEFAULT_SWIPE_THRESHOLD) {
                    //if not at the end of the pages, you don't want to try and advance into nothing!
                    if (mCurrentPage < (numberOfPages - 1)) {
                        edgePosition = (fingerUpPage + 1) * mPageHeight;
                    } else {
                        edgePosition = mCurrentPage * mPageHeight;
                    }
                ...
                if (mDistanceY < -DEFAULT_SWIPE_THRESHOLD) {
                    Log.i("####", "backwards");
                    edgePosition = (int) (fingerUpPage) * mPageHeight;
                    //return to start position
                } else {
                    Log.i("####", "backwards else");
                    //if at beginning, correct
                    if (Math.round(fingerUpPage) == 0) {
                        Log.i("####", "backwards correct");
                        //need to correct for when user starts to scroll into
                        //nothing then pulls it back a bit, this becomes a
            ...
            smoothScrollToPage((int) edgePosition / mPageHeight, forward);
